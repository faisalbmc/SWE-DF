...
#ifndef FWAVEVECTRACE_HPP_
#define FWAVEVECTRACE_HPP_
...
public:
	...
	FWaveVecTrace(T i_dryTol = (T) 1.0,
				 T i_gravity = (T) 9.81,
	...
	void computeNetUpdates ( T i_hLeft,  T i_hRight,
                             ...
                             T &o_huUpdateLeft,
                             T &o_huUpdateRight,
                             T &o_maxWaveSpeed,int traceon ) const
	{

		if (traceon!=0) std::cout << "pre_hLefty " << i_hLeft << "\n" << flush;
		if (traceon!=0) std::cout << "pre_hRight " << i_hRight << "\n" << flush;
		if (traceon!=0) std::cout << "pre huleft " << i_huLeft << "\n" << flush;
		if (traceon!=0) std::cout << "pre_huRight " << i_huRight << "\n" << flush;
		if (traceon!=0) std::cout << "pre_bleft " << i_bLeft << "\n" << flush;
		if (traceon!=0) std::cout << "pre bright " << i_bRight << "\n" << flush;
		...
		// determine the wet dry state and corr. values, if necessary.
		  if( i_hLeft >= dryTol ) {
			...
			if (traceon!=0) std::cout << "pre2_hLefty " << i_hLeft << "\n" << flush;
			if (traceon!=0) std::cout << "pre2_hRight " << i_hRight << "\n" << flush;
			if (traceon!=0) std::cout << "pre2 huleft " << i_huLeft << "\n" << flush;
			if (traceon!=0) std::cout << "pre2_huRight " << i_huRight << "\n" << flush;
			if (traceon!=0) std::cout << "pre2_bleft " << i_bLeft << "\n" << flush;
			if (traceon!=0) std::cout << "pre2 bright " << i_bRight << "\n" << flush;
		  ...
		  fWaveComputeWaveSpeeds( i_hLeft, i_hRight,
		                          ...
		                          waveSpeeds0, waveSpeeds1,traceon );         // 20 FLOPs (incl. 3 sqrt, 1 div, 2 min/max)
		  if (traceon!=0) std::cout << "waveSpeeds0 " << waveSpeeds0 << "\n" << flush;
		  if (traceon!=0) std::cout << "waveSpeeds1 " << waveSpeeds1 << "\n" << flush;
		  ...
		  fWaveComputeWaveDecomposition( i_hLeft, i_hRight,
		                                 ...
		                                 fWaves0, fWaves1,traceon);           // 23 FLOPs (incl. 1 div)

		  if (traceon!=0) std::cout << flush << "fWaves0 " << fWaves0 << "\n" << flush;
		  if (traceon!=0) std::cout << flush << "fWaves1 " << fWaves1 << "\n" << flush;
		 ...
		 // if (traceon!=0) std::cout << flush << "o_hUpdateLeft " << o_hUpdateLeft << "\n" << flush;
		 // if (traceon!=0) std::cout << flush << "o_hUpdateRight " << o_hUpdateRight << "\n" << flush;
		 // if (traceon!=0) std::cout << flush << "o_huUpdateLeft " << o_hUpdateLeft << "\n" << flush;
		  //if (traceon!=0) std::cout << flush << "o_huUpdateRight " << o_huUpdateRight << "\n" << flush;

		  //compute maximum wave speed (-> CFL-condition)
		  o_maxWaveSpeed = std::max( std::abs(waveSpeeds0) , std::abs(waveSpeeds1) );
		  if (traceon!=0) std::cout << flush << "o_maxWaveSpeed " << o_maxWaveSpeed << "\n" << flush;
		                                                              // 3 FLOPs (2 abs, 1 max)
		                                                              //========================
	...
	void fWaveComputeWaveSpeeds(
            ...
            T &o_waveSpeed0, T &o_waveSpeed1,int traceon ) const
	{
		...
		if (traceon!=0) std::cout << flush << "characteristicSpeed0 " << characteristicSpeed0 << "\n" << flush;
		if (traceon!=0) std::cout << flush << "characteristicSpeed1 " << characteristicSpeed1 << "\n" << flush;
	...
	void fWaveComputeWaveDecomposition(
			...
			T &o_fWave0, T &o_fWave1,int traceon ) const
	{
	  ...
	  //if (traceon!=0) std::cout << "dif "<<fDif1 << "\n";
	  // \deta x \Psi[2]
	  if (traceon!=0) std::cout << flush << "fDif1 " << fDif1 << "\n" << flush;
	  fDif1 += half_gravity * (i_hRight + i_hLeft)*(i_bRight - i_bLeft);     // 5 FLOPs
	  if (traceon!=0) std::cout << flush << "i_hRight " << i_hRight << "\n" << flush;
	  if (traceon!=0) std::cout << flush << "i_hLeft " << i_hLeft << "\n" << flush;
	  if (traceon!=0) std::cout << flush << "i_bRight " << i_bRight << "\n" << flush;
	  if (traceon!=0) std::cout << flush << "i_bLeft " << i_bLeft << "\n" << flush;
	  ...
	  if (traceon!=0) std::cout << flush << "fDif0 " << fDif0 << "\n" << flush;
	if (traceon!=0) std::cout << flush << "fDif1 " << fDif1 << "\n" << flush;
	 if (traceon!=0) std::cout << flush << "inverseSpeedDiff " << inverseSpeedDiff << "\n" << flush;
	  // compute f-waves:
