 ...
 *
 */
SWE_WaveAccumulationBlock::SWE_WaveAccumulationBlock(int l_nx, int l_ny,
		float l_dx, float l_dy) :
		SWE_Block(l_nx, l_ny, l_dx, l_dy), hNetUpdates(nx + 2, ny + 2), huNetUpdates(
				nx + 2, ny + 2), hvNetUpdates(nx + 2, ny + 2) {
}
...
//{
	...
	//change: use reduction instead of critical
#pragma omp parallel for reduction(max:l_maxWaveSpeed)
#endif // LOOP_OPENMP



	for (int xblock_count = 0; xblock_count < nblocks; xblock_count++) {
		for (int yblock_count = 0; yblock_count < nblocks; yblock_count++) {
			//i < nx + 2
			int endy= xblock_count!=nblocks-1 ? (xblock_count+1)*blocksize +1 : ny+2;
			for (int i = 1+xblock_count*blocksize; i <endy ; i++) {

#ifdef VECTORIZE // Vectorize the inner loop
#pragma simd
#endif // VECTORIZE
			for (int j = yblock_count*blocksize+1; j < (yblock_count+1)*blocksize +1  ; j++) {
			...
			wavePropagationSolver.computeNetUpdates(h[i - 1][j], h[i][j],
					hu[i - 1][j], hu[i][j], b[i - 1][j], b[i][j], hNetUpLeft,
					hNetUpRight, huNetUpLeft, huNetUpRight, maxEdgeSpeed);
...
#pragma omp parallel for reduction(max:l_maxWaveSpeed)
	...
	for (int xblock_count = 0; xblock_count < nblocks; xblock_count++) {
		...
		for (int yblock_count = 0; yblock_count < nblocks; yblock_count++) {
			//i < nx + 2
			for (int i = 1 + xblock_count * blocksize; i < (xblock_count + 1) * blocksize + 1; i++) {
				int endx = 	xblock_count != nblocks - 1 ? (yblock_count + 1) * blocksize + 1 : nx + 2;
#ifdef VECTORIZE // Vectorize the inner loop
#pragma simd
#endif // VECTORIZE
				//for(int j = 1; j < ny_end; j++) {

				for (int j = yblock_count * blocksize + 1; j < endx; j++) {
					float maxEdgeSpeed;
					float hNetUpDow, hNetUpUpw;
					float hvNetUpDow, hvNetUpUpw;

					wavePropagationSolver.computeNetUpdates(h[i][j - 1],
							h[i][j], hv[i][j - 1], hv[i][j], b[i][j - 1],
							b[i][j], hNetUpDow, hNetUpUpw, hvNetUpDow,
							hvNetUpUpw, maxEdgeSpeed);
...
#pragma omp parallel for
	...
	for (int i = 1; i < nx + 1; i++) {
		...
		//change use simd instead of ivdep
#pragma simd
#endif // VECTORIZE
		for (int j = 1; j < ny + 1; j++) {
			...
			if (h[i][j] < 0) {
				...
				if (h[i][j] < -0.1) {
					std::cerr << "Warning, negative height: (i,j)=(" << i << ","
							<< j << ")=" << h[i][j] << std::endl;
